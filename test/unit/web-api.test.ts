...
// errText 是 5 条固定兜底文案（发送/接管/结束/创建/撤销失败）的唯一收口。
describe('errText', () => {
  it('服务端给了 message 就用服务端文案', () => {
    expect(errText(new ApiError(400, 'BAD_REQUEST', '该会话未托管'), '发送失败')).toBe('该会话未托管');
  });
  ...
  it('ApiError 的 message 为空串时用兜底', () => {
    expect(errText(new ApiError(400, 'UNKNOWN', ''), '发送失败')).toBe('发送失败');
  });
